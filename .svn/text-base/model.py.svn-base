...
db_master = database.Connection("%s:%s"%(MYSQL_HOST_M,str(MYSQL_PORT)), MYSQL_DB,MYSQL_USER, MYSQL_PASS, max_idle_time = 5) 
db_slave  = database.Connection("%s:%s"%(MYSQL_HOST_S,str(MYSQL_PORT)), MYSQL_DB,MYSQL_USER, MYSQL_PASS, max_idle_time = 5)
...
class Category(object):
    ...
    def add_category(self,categoryname,update_postcount = False):
        ...
        db_master._ensure_connected()
        if not rtn:
            rtn= db_master.execute_lastrowid("insert into `py_category`(category,postcount) values (%s,0)",categoryname)
...
class Post(object):
    ...
    #@cacheData("post_{page}_{pagesize}_{tag}_{category}_{showall}_{keyword}_{onlypost}")
    ...
    def edit_post(self,**params):
        ...
        """
        rowcount = db_master.execute_rowcount(sql,params['title'],params['content'],params['status'],params['password'],params['commentstatus'],int(time.time()),params['posttype'],params['alias'], params['id'])
        if rowcount==1:
            if params['tag']:
                db_master._ensure_connected()
                ...
                for tag in params['tag']:
                    ...
                    if tagid:                 
                        db_master._ensure_connected()
                        db_master.execute("INSERT INTO `py_post_tag` (`postid`, `tagid`) VALUES (%s,%s)",params['id'],tagid)
            if params['category']:
                db_master._ensure_connected()
                ...
                for category in params['category']:
                    ...
                    if categoryid:
                        db_master._ensure_connected()
                        db_master.execute("INSERT INTO `py_post_category` (`postid`, `categoryid`) VALUES (%s,%s)",params['id'],categoryid)
    ...
    def new_post(self,**params):
        ...
        if postId:
            ...
            for category in params['category']:
                ...
                if categoryid:
                    db_master._ensure_connected()
                    db_master.execute("INSERT INTO `py_post_category` (`postid`, `categoryid`) VALUES (%s,%s)",postId,categoryid)
            return postId
        return 0
    ...
    def delete_post_by_id(self,id):
        ...
        for tag in tags:
            db_master.execute("update py_tags set postcount = postcount - 1 where id=%s and postcount > 0",tag['tagid'])
        for category in categorys:
            db_master.execute("update py_category set postcount = postcount - 1 where id=%s and postcount > 0",category['categoryid'])
    ...
    #@cacheData("get_post_by_alias_{alias}_{ignorestatus}")
    ...
    #@cacheData("get_post_by_id_{postid}_{ignorestatus}")
