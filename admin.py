...
class AdminAddPost(BaseHandler):
    ...
    def post(self):
        ...
        if title == '' or content == '':
            ...
            postid = Post.new_post(title=title, content=content, status=status, commentstatus=commentstatus,
                                   password=password, tag=tag, category=category, alias=alias, posttype=posttype)
            if postid:
...
class Settings(BaseHandler):
    ...
    def post(self):
        ...
        BlogConfig.update_configs(title=title, subtitle=subtitle, pagesize=pagesize, enablecomment=enablecomment,
                                  isclose=isclose)
        self.flash(u'配置更新成功')
