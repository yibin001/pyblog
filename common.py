    ...
    def _decorate(method):
        def _wrapper(*args, **kwargs):
            ...
            if html:
                ...
                request.set_header('Expires', '%s' % (
                    time.strftime('%a, %d %b %Y %H:%M:%S GMT', time.gmtime(time.time() + expire_time))))
                request.write(html)
            else:
...
def get_location_byip(ip):
    ...
    if response.status == 200:
        try:
            ...
            if jsonstring['ret'] == 1:
                return '%s%s%s%s' % (
                    jsonstring['country'], jsonstring['province'], jsonstring['city'], jsonstring['isp'])
        except Exception, e:
...
def spam_check(content, ip):
    ...
    try:
        ...
        if real_key:
            is_spam = akismet.comment_check(my_api_key, blogconfig['site_domain'], ip,
                                            """Mozilla/5.0 (...) Gecko/20051111 Firefox/1.5""", comment_content=content)
...
def Authentication(url='/admin/login/'):
    """
    用户身份认证
    @param url:login url to redirect.....
    @return:
    """
...
class BaseHandler(tornado.web.RequestHandler, flash_message.FlashMessage):
    ...
    @property
    def db(self):
        return torndb.Connection(MYSQL_HOST_M + ':' + MYSQL_PORT, MYSQL_DB, user=MYSQL_USER, password=MYSQL_PASS,
                                 time_zone='+8:00')
